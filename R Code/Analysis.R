...
price_df <- ricewheatmaize %>% left_join(pricedata) %>% 
            rename(ProducerPrice_USD='Producer price',ReferencePrice_USD='Reference price') %>% 
             mutate(ProducerPrice_LCU=ProducerPrice_USD*Exchange_Rate,ReferencePrice_LCU=ReferencePrice_USD*Exchange_Rate) %>% 
             select(CountryName,CountryCode,Year,Product,ProducerPrice_USD,ProducerPrice_LCU,
                    ReferencePrice_USD,ReferencePrice_LCU,Exchange_Rate) %>% 
             arrange(CountryName,Product,Year)


write.csv(price_df,'./NRA_Output/PriceData_USD_LCU.csv', row.names = F)
