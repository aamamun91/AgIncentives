...
## Files have to be be imported manually to the source folder
 SOURCE_OECD <- read.csv("./Source/Input_files_2025/OECD_input_file.csv", stringsAsFactors=FALSE)
 TIME_OECD <-file.info("./Source/Input_files_2025/OECD_input_file.csv")$ctime
 SOURCE_WBSA <- read.csv("./Source/Input_files_2025/WBSA_input_file.csv", stringsAsFactors=FALSE)
 TIME_WBSA <-file.info("./Source/Input_files_2025/WBSA_input_file.csv")$ctime
 SOURCE_FAO <- read.csv("./Source/Input_files_2025/MAFAP_input_file.csv", stringsAsFactors=FALSE)
 TIME_FAO <-file.info("./Source/Input_files_2025/MAFAP_input_file.csv")$ctime
 SOURCE_IDB <- read.csv("./Source/Input_files_2025/IDB_input_file.csv", stringsAsFactors=FALSE) %>%
...
SOURCE_VERSION  = c("OECD (PSE December 2025)", "FAO (December 2025)", 
                    "Agrimonitor (IDB, December 2025)",
...
write.csv(AGDATA, paste0("./NRP/Intermediary/AGDATA-", Sys.Date(), ".csv", sep = ""), row.names = FALSE)
}
###############################################################################
# Exchange Rate Table ---------------------------------------------------------
{
FOREX <-  AGDATA %>%
          select(SOURCE,
                 ...
                 ER_OFFICIAL, 
                 EROFFICIAL_SOURCE,
                 EROFFICIAL_UNIT) %>% filter(EROFFICIAL_SOURCE!='RBI') %>% distinct()
...
write.csv(FOREX,'ExchangeRate_Table_2025.csv')
###############################################################################
...
# pricedata <- CONS_MAPPED0 %>% 
#              select(COUNTRY_CODE, YEAR,AGCOMNAME,PROP,REFP,ER_OFFICIAL, EROFFICIAL_SOURCE) %>% 
#              filter(AGCOMNAME %in% c('Wheat','Rice, paddy','Maize')) %>% 
#              mutate(AGCOMNAME=ifelse(AGCOMNAME=='Rice, paddy','Rice',AGCOMNAME)) %>% 
#              rename(CountryCode=COUNTRY_CODE,Year=YEAR,Product=AGCOMNAME,'Reference price'=REFP,
#                     'Producer price'=PROP,Exchange_Rate=ER_OFFICIAL) %>% select(-'Producer price',-'Reference price', -Product) %>% 
#              distinct() %>% filter(EROFFICIAL_SOURCE!='RBI')
...
TEST_NOTECASE <- TEMP_NOTECASEA %>%
                ...
                summarize(FREQ=n()) %>%
                ungroup()
write.csv(TEST_NOTECASE, "./NRP/Tests/TEST_CASESvsNOTES.csv")
...
write.csv(CHECK_COUNTRYPROD, "./NRP/Tests/TEST_COUNTRYxPRODUCT.csv")
...
save(TPUBLICVIEW,file="./NRP/TPUBLICVIEW.Rdata")
