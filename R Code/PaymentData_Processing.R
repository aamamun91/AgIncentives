...
library(tidyverse)
library(stringr)
source("R Code/Mapping.R")
source("R Code/VoP_NonMPS_Processing.R")


# Data loading --------------------------------------------------------------------------------
## first, payments data file
PAYMENTS <- read.csv(file="Source/PaymentData_2025/Consolidated_Payment_Data.csv") %>%
            dplyr::filter(Year>=2005) %>%
            mutate(Source=ifelse(Source %in% c("OECD","FAO","IDB"), Source, "OthSources"), 
                   Source=ifelse(Source=='IDB', 'Agrimonitor (IDB)', Source)) %>%
            dplyr::filter(Year<=2024) %>% filter(Source!='OthSources') 

## NRP database, currently online 
NRP_CSV <- read.csv(file="Source/PUBLICVIEW_2025/PUBLICVIEW.csv")%>%
            dplyr::filter(Category=="COUNTRY_PRODUCT")

NRP_DET <- read.csv(file="Source/PUBLICVIEW_2025/PUBLICVIEW.csv")%>%
           dplyr::filter(Category=="COUNTRY_PRODUCT") %>% 
...
NRP_TOTAL <- read.csv(file="Source/PUBLICVIEW_2025/PUBLICVIEW.csv")%>%
...
EU27 <- read.table("Mapping/EU27.txt",col.names = FALSE)%>%flatten%>%unlist
EU28 <- read.table("Mapping/EU28.txt",col.names = FALSE)%>%flatten%>%unlist
...
faostat.vop <- read.csv(".\\Source\\FAOSTAT\\Data_2025\\Value_of_Production_E_All_Data_(Normalized).csv") %>%
                rename(VoPFAO=Value, element=Element, element_code=Element.Code) %>% 
                filter(Year>=2005 & Year<=2024) %>% left_join(COUNTRYFAO) %>% rename(CountryCode=ISO3_CODE) 
                       ...
                       mutate(Commodity_Label=paste0('Non-MPS other crops - ', Country_Label),
                        ...
                        group_by(Source, Country_Label, Country_Code, Commodity_Label, Commodity_Code,Commodity_Type,    
                         ...
                         summarise(Support_USD=sum(Support_USD, na.rm = T)) %>% ungroup() %>% select(-RelevantValueProduction) |> 
                          mutate(Country_Label=case_when(Country_Label=='Bolivia'~'Bolivia, Plurinational State of', 
                                                         TRUE~Country_Label
                          )) |> 
                       mutate(Commodity_Label=ifelse(Commodity_Label=='Non-MPS other crops - Bolivia',
                                                     'Non-MPS other crops - Bolivia, Plurinational State of', Commodity_Label))
...
dir.create(paste0("NRA/Final/",Sys.Date(), sep = ""))

readr::write_excel_csv(CONSOLIDATED_NRA, paste0("NRA/Final/", Sys.Date(),"/Detailed_NRA.csv", sep = ""))
...
faostat.production <- read.csv("Source/FAOSTAT/Data_2025/Production_Crops_Livestock_E_All_Data_(Normalized).csv") %>%
...
check.production <- missing.tuples %>% left_join(faostat.production, by=c('Country_Label'='Area', 'Commodity_Label'='Item', 
                                                                          'Item.Code'='Item.Code', 'Year'='Year')) %>% 
                    filter(!is.na(Value))

readr::write_excel_csv(check.production, 'NRA/test/Country_Commodity_Missing_Tuples.csv', na='')
...
readr::write_excel_csv(countrylist.bysource, "NRA/test/Coverage_Country_List.csv")  
...
VoP_FAOSTAT.coverage <- VoP_FAOSTAT.final %>% group_by(Year) %>% 
...
readr::write_excel_csv(VoP_FAOSTAT.final, "NRA/test/VOP_Final.csv") 
readr::write_excel_csv(VoP_FAOSTAT.coverage, "NRA/test/VoP_FAOSTAT.csv")  
...
readr::write_excel_csv(AGGREGATED_NRA, paste0("NRA/Final/", Sys.Date(),"/Aggregated_NRA.csv", sep = ""))
